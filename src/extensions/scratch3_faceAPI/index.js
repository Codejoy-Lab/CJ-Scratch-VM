...
;
    ...
    getemotion(){
        ...
        if(faceDetectionResult) {
            var emotion = {confidence:0,item:""}
            Object.keys(faceDetectionResult.expressions).forEach(function(key){
                if(faceDetectionResult.expressions[key]>emotion.confidence){
                    emotion.confidence = faceDetectionResult.expressions[key]
                    emotion.item = key
                }
           });
            // console.log('faceDetectionResult.expressions.neutural',faceDetectionResult.expressions.neutral)
            return emotion.item
        }
    }
    ...
    faceDetection()
	{
        ...
        //     originCanvas.parentElement.append(canvas)
            ...
            this.timer = setInterval(async () => {
                ...
                console.log('faceDetectionResult',faceDetectionResult)
                // 确认仅得到数据后进行绘制
                // if (results) {
