...
class cjfaceapi {
    ...
    reset() {
        ...
        for (let i = 0; i < targets.length; i++) {
            const state = targets[i].getCustomState(cjfaceapi .STATE_KEY);
            if (state) {
     ...
     */
    _loop() {
        setTimeout(this._loop.bind(this), Math.max(this.runtime.currentStepTime, cjfaceapi .INTERVAL));
        ...
        if (offset > cjfaceapi .INTERVAL) {
            const frame = this.runtime.ioDevices.video.getFrame({
                format: Video.FORMAT_IMAGE_DATA,
                dimensions: cjfaceapi .DIMENSIONS
            });
            if (frame) {
                this._lastUpdate = time;
            }
    ...
    _getMotionState(target) {
        let motionState = target.getCustomState(cjfaceapi .STATE_KEY);
        if (!motionState) {
            motionState = Clone.simple(cjfaceapi .DEFAULT_MOTION_STATE);
            target.setCustomState(cjfaceapi .STATE_KEY, motionState);
        }
        return motionState;
    }
    ...
    async knnInit () {
        ...
        console.log("faceapi.nets",faceapi.nets) //控制台显示
    }
    
}

module.exports = cjfaceapi;